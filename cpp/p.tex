...
good notion of homomorphism between them. This is why Kaposi and Xie
...
models witha initial model (syntax), (co)free models
...
of the function to make sense. This is the type version of the
...
this is why we had to lift $\El\,\q$. Another subtlety is
...
names to the roundtrips according to our convention introduced in
Section \ref{sec:metatheory}. xxx

but
naturality was only stated for the right to left direction (rule
${,}[]$). Naturalities in the direction are derivable:
\begin{alignat*}{10}
  & (\fst\,w)[\gamma] && {=}(xxx) \\
  & \fst\,((\fst\,w)[\gamma],(\snd\,w)[\gamma]) = \\
  & \fst\,((\fst\,w,\snd\,w)[\gamma]) = \\
  & \fst\,(w[\gamma])
\end{alignat*}

We show the commutation of $\Lift$ and $\Pi$ types as a sequence of
isomorphisms:
\begin{alignat*}{10}
  & \Tm\,\Gamma\,(\Lift\,(\Pi\,A\,B)) && {\cong}\text{(1)} \\
  & \Tm\,\Gamma\,(\Pi\,A\,B) && {\cong}\text{(2)} \\
  & \Tm\,(\Gamma\ext A)\,B && {\cong}\text{(3)} \\
  & \Tm\,(\Gamma\ext A)\,(\Lift\,B) && {\cong}\text{(4)} \\
  & \Tm\,(\Gamma\ext \Lift\,A)\,(\Lift\,B[\p^+][\langle\un\,\q\rangle]) && {\cong}\text{(5)} \\
  & \Tm\,\Gamma\,(\Pi\,(\Lift\,A)\,(\Lift\,B[\p^+][\langle\un\,\q\rangle]))
\end{alignat*}
Steps (1) and (3) are the isomorphism comprised of $\mk$ and $\un$. 
