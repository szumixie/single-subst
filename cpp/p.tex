...
Some advantages of using a GAT presentation:
\begin{itemize}
\item  In type theory, typing depends on conversion:
  in an extrinsic presentation, this is expressed by a rule such as
  the following.
  \[
  \infer{\Gamma\vdash t : B}{A \sim B && \Gamma\vdash t : A}
  \]
  When conversion is equality, this rule holds by the general
...
\item The GAT approach is also more abstract than extrinsic
  ...
  with paranoid or economic typing rules \cite[Section
    5]{winterhalter:hal-01626651}, or whether we have explicit or
  ...
  type, se we are always {\`a} la Church; algebraic theories don't
...
context \emph{at the same time}, that is, $t[x_0\mapsto
  u_0,\dots,x_n\mapsto u_n]$ where $x_0,\dots,x_n$ are all the free
variables in $t$. $(x_0\mapsto u_0,\dots,x_n\mapsto u_n)$ is called a
parallel substitution, and the terms $u_0,\dots,u_n$ are all in the
same context. Parallel substitutions are very natural in the algebraic
setting because they form the morphisms in a category where objects
are contexts (lists of types), and then it is clear what equations
they need to satisfy. Some examples of algebraic definitions of type
theory using parallel substitutions ranging from the more syntactic to
the more semantic: Ehrhard's notion \cite{ehrhard,coquandEhrhard},
categories with families (CwF
\cite{DBLP:conf/types/Dybjer95,Castellan2021}), Awodey's natural
models \cite{DBLP:journals/mscs/Awodey18}, contextual categories
\cite{DBLP:journals/apal/Cartmell86}, C-systems
...
simply replaces all free occurrences of a variable $x$ with a term $u$
as in the notation $t[x\mapsto u]$. A substitutions consists of a
...
which is simpler and more minimalistic than the existing algebraic
...
that the \emph{syntax} of our single substitution theory and the
syntax of the corresponding parallel substitution theory (CwF-based
theory) are isomorphic. All CwF-based models are models of our single
substitution theory, but not necessarily the other way. While
B-systems are equivalent to CwFs (and C-systems
\cite{AHRENS_EMMENEGGER_NORTH_RIJKE_2023}), there are more SSC models
than CwF-based models. This relationship is similar to that of
extensional combinatory calculus and lambda calculus, where the
...
not like ordinary normal forms because they are still quotiented by
the computation/uniqueness rules like $\beta$/$\eta$ for function
space. We use induction on $\alpha$-normal forms to define parallel
substitutions and prove all the rules of CwFs in the syntax of SSC.

Our SSC has 8 equations, and these are enough to describe type
theories with arbitrary choice of type formers (see the description of
...
4 (conditional) equations on variables are enough, because using the
fact that every type is represented by a terma, we can derive the other
4 equations.
...
parallel substitutions, functions into the universe represent depenent
types. Then the SSC is just there to formulate $\Sigma$, $\Pi$, $\U$,
and then we use these to define a parallel substitution calculus. So
...
All of our constructions can be understood as happening internal to
extenional type theory, and most results in this paper are formalised
in Agda. Most results in this paper were presented at the TYPES 2024
...
After discussing related work, in Section \ref{sec:tt}, we introduce
our single substitution calculus (SSC) only relying on a working
knowledge of an implementation of type theory such as Agda, no prior
experience with the metatheory of type theory is required. Section
\ref{sec:admissible} shows that in the syntax of SSC, several new
equations are admissible, and that the syntax of SSC is actually
isomorphic to the CwF-based syntax. In Section \ref{sec:minimisation}
we minimise our SSC obtaining a theory with fewer equations, relying
on the presence of $\Pi$ and $\U$ in our theory. In Section
\ref{sec:cwf}, we show that in the presence of $\Pi$, $\U$ and
$\Sigma$, SSC-models are actually equivalent to CwF-models in a weak
sense. We conclude in Section \ref{sec:conclusion}.
...
substitutions. B-systems correspond to our SSC as essentially
algebraic theories correspond to generalised algebraic theories, or
sets with a map into $I$ correspond to indexed families over $I$
...
Agda using postulated quotients. Although there is no general proof
published about this, we have experimental evidence that QIITs are
supported by the cubical set model \cite{DBLP:conf/lics/CoquandHM18}
and the setoid model \cite{kaposi-qiit-setoid}, thus Cubical Agda
...
theory even more abstractly than GATs: they even abstract over the way
of defining the substitution calculus: contexts, substitutions,
instantiations are simply modelled by the metatheoretic function
space. For example, a type theory with $\Pi$, Coquand-universes and a
universe level lifting operation is described by the following SOGAT
(where $\cong$ means isomorphism):
...
% \end{alignedat}
% \end{equation}
Second-order models of SOGATs are not well-behaved as there is no good
notion of homomorphism between them. This is why Kaposi and Xie
\cite{DBLP:conf/fscd/KaposiX24} translate SOGATs to GATs: each GAT
comes with a category of models with initial model (syntax), (co)free
models, and so on. Actually, Kaposi and Xie define two different
translations: one based on parallel substitutions and one based on
single substitutions. If we apply their parallel translation to the
SOGAT
...
of category with families (CwF). If we use their single substitution
translation on the SOGAT (\ref{eq:tt}), we obtain Definition
\ref{def:tt} (also listed in Appendix \ref{app:tt}). This paper can be
seen as taking a big enough type theory (described as a SOGAT), and
investigating how the result of its single substitution translation
relates to the result of its parallel translation (the latter
first-order theory is quite well-known).
