...
In our single substitution calculus, we have the equation $[\p][^+]$
...
code in the universe, so for any $B : \Ty\,\Gamma\,i$,
\begin{alignat*}{10}
  & B[\p][\gamma^+] && {=}(\U\beta) \\
  & \El\,(\cd\,B)[\p][\gamma^+] && {=}(\El[]) \\
  & \El\,((\cd\,B)[\p][\gamma^+])\,\, && {=}([\p][^+]') \\
  ...
  & B[\gamma][\p].
\end{alignat*}
Equation $[\q][^+] : \q[\gamma^+] = \q$ only makes sense if we have
$[\p][^+]$ for types, so either we make $[\q][^+]$ conditional, or we
coerce it along the derived equation.
...
\section{The single substitution GAT presentation of the SOGAT (\ref{eq:tt})}

For reference, we list the rules of the theory introduced in Section
\ref{sec:ssc}. The core substitution calculus:
\begin{alignat*}{10}
& \Con && : \Set \\
& \Ty && : \Con\ra\N\ra\Set \\
& \diamond && : \Con \\
