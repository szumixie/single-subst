...
TODO. In Section \ref{sec:ssc}, we introduce the single substitution
...
theories close to the notations using typing judgements. B-systems are
a generalised algebraic (intrinsic) presentation of type theory using
single substitutions. xxx

\begin{tikzpicture}
  \node (b0) at (0,0) {$\top\cong B_0$};
  \node (b1) at (2,0) {$B_1$};
  \node (b2) at (4,0) {$B_2$};
  \node (b3) at (6,0) {$\dots$};
  \node (c1) at (3,1) {$\tilde{B}_1$};
...
Using our
notations B-systems can be described as follows. B-frame. TODO
...
$\blank,\blank$ operations which makes it more apparent that
substitutions are simple lists of terms. CwFs are equivalent to
Ehrhard's calculus, and so are contextual categories/C-systems
...
Kaposi and Luksa \cite{} defined a telescopic single substitution
...
Most formalisations of type theory are extrinsic
\cite{pujet,abel,metacoq}: they define the syntax as abstract syntax
trees, and equip it with typing and conversion relations. A higher
level representation is working in setoid hell
\cite{chapman09eatitself}: terms are intrinsically well-typed, but
conversion is still an explicit relation. For formalising the
GAT-level syntax, one needs a stronger metatheory than ordinary Agda
or Coq: the metatheory has to support quotient inductive-inductive
types (QIITs \cite{DBLP:journals/pacmpl/KaposiKA19}), in other words,
initial models of GATs. Altenkirch and Kaposi
...
set model \cite{} and the setoid model \cite{kaposi-qiit-setoid}, thus
Cubical Agda \cite{} and setoid/observational type theory
\cite{setoid,loicThesis} support QIITs. For example, in Cubical Agda,
set-truncated and groupoid-truncated syntaxes of type theory have been
formalised \cite{cohtt}. Without QIITs, tricks such as shallow
embedding the syntax can be used to formalise metatheoretic results
about type theory such as gluing
\cite{kaposi_et_al:LIPIcs:2019:10532}, and its special cases
canonicity and parametricity \cite{kaposi-shallow}.

At the level of abstraction of SOGATs, the difference between single
...
no need for parallel substitutions, empty substitution, parallel
weakenings, telescopes, or combinations of these.

say that we assume the QIIT version of this.
